...
def get_top_words(file_path,topn):
    # 为了性能考虑，如果当前目录下有cut_result.txt文件，则从cut_result.txt中读取带词性的分词结果列表
    if os.path.exists('./cut_result.txt'):
...
def main():
    ...
    # 生成词云图片，bg.png是当前目录下的一副背景图片，yahei.ttf是当前目录下微软雅黑字体文件，santi_cloud.png是要生成的词云图片名
    generate_word_cloud('./bg.png',top_words_with_freq,'./yahei.ttf','./santi_cloud.png')
